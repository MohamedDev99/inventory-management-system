...
public class StockAdjustmentService {
        ...
        public StockAdjustmentResponse createAdjustment(StockAdjustmentRequest request, Authentication authentication) {
                ...
                InventoryItem inventory = inventoryItemRepository.findByProductAndWarehouse(product, warehouse)
                                .orElseThrow(
                                                () -> new InventoryItemNotFoundException(request.getProductId(),
        ...
        public Page<StockAdjustmentResponse> getAllAdjustments(
                ...
                return stockAdjustmentRepository
                                .findAllWithFilters(productId, warehouseId, status, reason, startDate, endDate,
                                                pageable)
                                .map(this::toResponse);
        }
        ...
        public StockAdjustmentResponse approveAdjustment(Long id, StockAdjustmentApproveRequest request,
                ...
                // Resolve the actor from the security context — never trust client-supplied
                // identity
                String username = authentication.getName();
                ...
                // 10% threshold rule: adjustments exceeding 10% of current stock value require
                // ADMIN
                ...
                if (!isAdmin) {
                        ...
                        if (changePercent > 10.0) {
                                throw new StockAdjustmentException(String.format(
                                                "Adjustment of %.1f%% exceeds the 10%% threshold. ADMIN approval is required.",
                                                changePercent));
                        }
                }
                ...
                log.info("Stock adjustment id={} approved by {}. Inventory updated: {} → {}",
                                id, username, currentStock, resultingStock);
        ...
        public StockAdjustmentResponse rejectAdjustment(Long id, StockAdjustmentRejectRequest request,
                        Authentication authentication) {
                log.info("Rejecting stock adjustment id={} by actor={}", id, authentication.getName());
                ...
                String rejectionNote = "REJECTED by " + username + ": " + request.getReason();
                ...
                log.info("Stock adjustment id={} rejected by {}.", id, username);
                return toResponse(adjustment);
        }
