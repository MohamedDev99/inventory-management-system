        ...
        for (ReceivePurchaseOrderRequest.ItemReceipt itemReceipt : request.getItems()) {
            ...
            if (totalReceived > item.getQuantityOrdered()) {
                throw new IllegalArgumentException(
                        String.format(
                                "Cannot receive more than ordered for item id %d. Ordered: %d, Already received: %d, Attempting to receive: %d",
                                item.getId(), item.getQuantityOrdered(), item.getQuantityReceived(),
                                itemReceipt.getQuantityReceived()));
            }
