...
-- Description: Product catalog with pricing, inventory settings, and optimistic locking
-- =========================================
...
CREATE INDEX idx_products_active_only ON products (id, name, category_id)
WHERE
    is_active = true;
...
COMMENT ON COLUMN products.version IS 'Optimistic lock version for preventing concurrent price/specification updates';

COMMENT ON COLUMN products.created_by IS 'Username of the user who created this product';

COMMENT ON COLUMN products.updated_by IS 'Username of the user who last updated this product';