...
import { stockTransferRequestSchema, stockAdjustmentRequestSchema, inventoryRequestSchema } from "@/lib/schemas/stock/request"
import type { PageParams } from "@/types"

// GET /api/inventory - Paginated inventory items
export async function getInventoryItems(params: PageParams & {
  warehouseId?: number
  productId?: number
  lowStock?: boolean
  search?: string
  ...
  const validated = validateResponse(
    ...
    response.data,
    { prefix: "Get Inventory Items Response" }
  )
  return validated
}
...
// GET /api/inventory/warehouse/{warehouseId} - All items in warehouse
...
export async function transferStock(data: Parameters<typeof stockTransferRequestSchema.parse>[0]) {
...
export async function adjustStock(data: Parameters<typeof stockAdjustmentRequestSchema.parse>[0]) {
...
export async function receiveInventory(data: {
  ...
  const validated = validateResponse(
    apiResponseSchema(z => z.any()),
    response.data,
    { prefix: "Receive Inventory Response" }
  )
  return validated
}
