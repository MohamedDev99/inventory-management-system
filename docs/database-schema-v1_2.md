...
| manager_id  | BIGINT        | FK â†’ users(id)              | Warehouse manager        |
...
| parent_category_id | BIGINT       | FK â†’ categories(id)         | Parent category (null for root) |
...
| category_id     | BIGINT        | NOT NULL, FK â†’ categories(id) | Product category                   |
...
| ---------------- | ----------- | ------------------------------------------ | ---------------------------------- |
| id               | BIGINT      | PRIMARY KEY, AUTO_INCREMENT                | Unique identifier                  |
| product_id       | BIGINT      | NOT NULL, FK â†’ products(id)                | Product reference                  |
| warehouse_id     | BIGINT      | NOT NULL, FK â†’ warehouses(id)              | Warehouse reference                |
...
| supplier_id            | BIGINT        | NOT NULL, FK â†’ suppliers(id)  | Supplier reference           |
| warehouse_id           | BIGINT        | NOT NULL, FK â†’ warehouses(id) | Destination warehouse        |
| created_by             | BIGINT        | NOT NULL, FK â†’ users(id)      | Order creator                |
...
| ----------------- | ------------- | --------------------------------------------------- | ------------------------- |
| id                | BIGINT        | PRIMARY KEY, AUTO_INCREMENT                         | Unique identifier         |
| purchase_order_id | BIGINT        | NOT NULL, FK â†’ purchase_orders(id)                  | PO reference              |
| product_id        | BIGINT        | NOT NULL, FK â†’ products(id)                         | Product reference         |
...
| customer_id      | BIGINT        | NOT NULL, FK â†’ customers(id)  | Customer reference           |
...
| warehouse_id     | BIGINT        | NOT NULL, FK â†’ warehouses(id) | Fulfillment warehouse        |
| created_by       | BIGINT        | NOT NULL, FK â†’ users(id)      | Order creator                |
...
| -------------- | ------------- | ------------------------------- | ------------------------- |
| id             | BIGINT        | PRIMARY KEY, AUTO_INCREMENT     | Unique identifier         |
| sales_order_id | BIGINT        | NOT NULL, FK â†’ sales_orders(id) | SO reference              |
| product_id     | BIGINT        | NOT NULL, FK â†’ products(id)     | Product reference         |
...
| id                | BIGINT      | PRIMARY KEY, AUTO_INCREMENT    | Unique identifier                          |
| product_id        | BIGINT      | NOT NULL, FK â†’ products(id)    | Product reference                          |
| from_warehouse_id | BIGINT      | FK â†’ warehouses(id)            | Source warehouse (null for receipts)       |
| to_warehouse_id   | BIGINT      | FK â†’ warehouses(id)            | Destination warehouse (null for shipments) |
...
| performed_by      | BIGINT      | NOT NULL, FK â†’ users(id)       | User who performed movement                |
...
| performed_by    | BIGINT       | NOT NULL, FK â†’ users(id)      | User who created adjustment |
| approved_by     | BIGINT       | FK â†’ users(id)                | User who approved           |
...
| performed_by | BIGINT      | NOT NULL, FK â†’ users(id)    | User who performed action                |
...
| ----------------- | ------------ | --------------------------- | -------------------- |
| id                | BIGINT       | PRIMARY KEY, AUTO_INCREMENT | Unique identifier    |
| user_id           | BIGINT       | NOT NULL, FK â†’ users(id)    | Recipient user       |
...
| generated_by | BIGINT       | NOT NULL, FK â†’ users(id)    | User who generated report  |
...
| manager_id           | BIGINT       | FK â†’ employees(id)          | Department manager     |
| parent_department_id | BIGINT       | FK â†’ departments(id)        | Parent department      |
...
| department_id    | BIGINT        | FK â†’ departments(id)        | Department          |
| manager_id       | BIGINT        | FK â†’ employees(id)          | Direct manager      |
| hire_date        | DATE          | NOT NULL                    | Date of hire        |
| termination_date | DATE          | -                           | Date of termination |
| salary           | DECIMAL(12,2) | -                           | Annual salary       |
| user_id          | BIGINT        | FK â†’ users(id)              | System user account |
...
| sales_order_id            | BIGINT        | NOT NULL, FK â†’ sales_orders(id) | Sales order reference         |
...
| shipped_from_warehouse_id | BIGINT        | NOT NULL, FK â†’ warehouses(id)   | Origin warehouse              |
| shipped_by                | BIGINT        | NOT NULL, FK â†’ users(id)        | User who shipped              |
...
| sales_order_id   | BIGINT        | FK â†’ sales_orders(id)        | Related sales order      |
| customer_id      | BIGINT        | NOT NULL, FK â†’ customers(id) | Customer                 |
...
| processed_by     | BIGINT        | NOT NULL, FK â†’ users(id)     | User who processed       |
| version       | BIGINT       | NOT NULL, DEFAULT 0         | Optimistic lock version   |
| created_at       | TIMESTAMP     | NOT NULL, DEFAULT NOW()      | Creation timestamp       |
| updated_at       | TIMESTAMP     | NOT NULL, DEFAULT NOW()      | Last update              |
| created_by    | VARCHAR(255) | NOT NULL,                   | username of user       |
...
| generated_by    | BIGINT        | NOT NULL, FK â†’ users(id)        | User who generated    |
| version       | BIGINT       | NOT NULL, DEFAULT 0         | Optimistic lock version   |
| created_at      | TIMESTAMP     | NOT NULL, DEFAULT NOW()         | Creation timestamp    |
| updated_at      | TIMESTAMP     | NOT NULL, DEFAULT NOW()         | Last update           |
| created_by    | VARCHAR(255) | NOT NULL,                   | username of user       |
...
#### WAREHOUSES â†’ INVENTORY_ITEMS (1:N)
...
#### EMPLOYEES â†” USERS (1:1 Optional)
...
#### EMPLOYEES â†’ EMPLOYEES (Self-referencing - Manager Hierarchy)
...
- Enables multi-level categorization (Electronics â†’ Computers â†’ Laptops)
...
- `DRAFT â†’ SUBMITTED`: Validate all line items, calculate totals
- `SUBMITTED â†’ APPROVED`: Requires MANAGER or ADMIN role
- `APPROVED â†’ RECEIVED`: Update inventory quantities, create movement records
- `Any â†’ CANCELLED`: Record cancellation reason, release any allocations
...
DELIVERED
  ...
  Any status â†’ CANCELLED (before SHIPPED)
```

**Transitions:**

- `PENDING â†’ CONFIRMED`: Check inventory, reserve stock
- `CONFIRMED â†’ FULFILLED`: Deduct inventory, create movement record
- `FULFILLED â†’ SHIPPED`: Generate shipping label, update tracking
- `SHIPPED â†’ DELIVERED`: Update delivery confirmation
- `Any â†’ CANCELLED` (before SHIPPED): Release reserved inventory

**Auto-transitions:**

- Pending â†’ Auto-cancel after 7 days of inactivity
- Shipped â†’ Delivered after tracking confirmation (integration)
...
- `PENDING â†’ APPROVED`: Requires MANAGER approval, apply adjustment to inventory
- `PENDING â†’ REJECTED`: Record rejection reason, no inventory change
...
- **Optimized optimistic locking** implementation (refined from v1.1.0)
- **Final: 15 tables with version field** (removed from 4 low-risk tables)
- Added `version BIGINT NOT NULL DEFAULT 0` column to:
  - Core entities: USERS, WAREHOUSES, PRODUCTS, INVENTORY_ITEMS, CUSTOMERS, EMPLOYEES (6 tables)
  - Transaction entities: PURCHASE_ORDERS, PURCHASE_ORDER_ITEMS, SALES_ORDERS, SALES_ORDER_ITEMS, INVENTORY_MOVEMENTS, STOCK_ADJUSTMENTS, SHIPMENTS, PAYMENTS, INVOICES (9 tables)
- **Excluded from versioning (7 tables):**
  - Static reference data: ROLES, CATEGORIES, SUPPLIERS, DEPARTMENTS
  - Append-only/low-priority: AUDIT_LOGS, NOTIFICATIONS, REPORTS
- **Rationale for exclusions:**
  - CATEGORIES: Static reference data, admin-only, infrequent changes
  - SUPPLIERS: Static reference data, low concurrency risk
  - DEPARTMENTS: Organizational data, centrally managed
  - REPORTS: Append-mostly, conflicts meaningless
- Added comprehensive "Optimistic Locking Strategy" section

### Version 1.1.0 (2026-01-27)

- Initial optimistic locking implementation (19 tables)
- Refined to 15 tables in v1.2.0
...
Optimistic locking prevents data corruption in concurrent multi-user environments by detecting conflicting updates before committing changes to the database.

### Implementation

15 tables include a `version` column for optimistic locking:
...
1. **Read:** Application reads a record including its current version number
   ```sql
   SELECT id, name, quantity, version FROM products WHERE id = 123;
   -- Returns: id=123, name="Laptop", quantity=50, version=5
   ```
...
3. **Outcomes:**
   - **Success:** If version matches, update proceeds and version increments to 6
   - **Conflict:** If version doesn't match (another user modified it), update affects 0 rows
   - Application detects conflict and can retry or notify user

### JPA Integration

Spring Data JPA provides automatic optimistic locking:
...
When a conflict occurs, JPA throws `OptimisticLockException`:
...
try {
    ...
    log.warn("Concurrent modification detected for product {}", product.getId());
    throw new ConcurrentModificationException("Product was modified by another user");
}
```

### Tables With Optimistic Locking (15 tables)

| Table                  | Rationale                                              |
| ---------------------- | ------------------------------------------------------ |
| USERS                  | Prevent concurrent profile updates                     |
| WAREHOUSES             | Protect capacity and configuration changes             |
| **PRODUCTS**           | **Critical:** Prices and specifications must be accurate |
| **INVENTORY_ITEMS**    | **Critical:** Prevent overselling and stock corruption |
| **PURCHASE_ORDERS**    | **Critical:** Financial data, prevent duplicate approvals |
| **PURCHASE_ORDER_ITEMS** | **Critical:** Prevent receiving quantity corruption  |
| **SALES_ORDERS**       | **Critical:** Financial data, prevent double fulfillment |
| **SALES_ORDER_ITEMS**  | **Critical:** Prevent shipping quantity corruption    |
| INVENTORY_MOVEMENTS    | Audit trail integrity, movement corrections            |
| **STOCK_ADJUSTMENTS**  | **Critical:** Prevent conflicting approval decisions  |
| CUSTOMERS              | Protect customer data and credit limits                |
| EMPLOYEES              | Personnel record accuracy, salary is financial data    |
| SHIPMENTS              | Tracking and delivery status consistency               |
| PAYMENTS               | Financial transaction integrity                        |
| INVOICES               | Billing accuracy and payment tracking                  |

### Tables WITHOUT Optimistic Locking (7 tables)

| Table                  | Rationale                                              |
| ---------------------- | ------------------------------------------------------ |
| ROLES                  | Static reference data, rarely updated                  |
| CATEGORIES             | Static reference data, infrequent changes              |
| SUPPLIERS              | Static reference data, low concurrency risk            |
| DEPARTMENTS            | Static organizational data, centrally managed          |
| REPORTS                | Append-mostly, conflicts are meaningless               |
| AUDIT_LOGS             | Append-only, never updated                             |
| NOTIFICATIONS          | Low priority, acceptable if deleted during read        |
...
When adding the `version` column to existing tables:

```sql
-- Add version column with default value 0
ALTER TABLE products ADD COLUMN version BIGINT NOT NULL DEFAULT 0;

-- For existing records, version starts at 0
-- Future updates will increment from there
```

### Performance Impact

- **Minimal overhead:** Single BIGINT column (8 bytes)
- **No additional queries:** Version checked in same UPDATE statement
- **No locks:** Optimistic approach avoids database locks
...
1. **Inventory Operations**
   - User A reads product quantity: 50
   - User B reads product quantity: 50
   - User A sells 10 units (quantity = 40, version = 2)
   - User B sells 15 units → **CONFLICT** (version mismatch)
   - User B must reload fresh data (quantity = 40) and retry

2. **Order Approval**
   - Manager A opens PO for approval (status=SUBMITTED, version=1)
   - Manager B opens same PO for approval (status=SUBMITTED, version=1)
   - Manager A approves (status=APPROVED, version=2)
   - Manager B tries to approve → **CONFLICT**
   - Prevented duplicate approval!

3. **Price Updates**
   - Admin A reads product price: $100 (version=5)
   - Admin B reads product price: $100 (version=5)
   - Admin A updates to $110 (version=6)
   - Admin B updates to $105 → **CONFLICT**
   - Admin B sees latest price ($110) before deciding

### Monitoring

Track optimistic lock failures in Grafana:

```promql
# Conflict rate by entity type
rate(database_optimistic_lock_failures_total[5m])

# Alert if conflict rate exceeds threshold
rate(database_optimistic_lock_failures_total[5m]) > 10
```
